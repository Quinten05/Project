  ...
  if (rx_buf_flag) {
    for (int x = 0; x < SLC_BUF_LEN; x++) {
      if (i2s_slc_buf_pntr[rx_buf_idx][x] > 0) {
        float sample = convert(i2s_slc_buf_pntr[rx_buf_idx][x]) / 4096.0f; // Genormaliseerd bereik
    ...
    if (numSamples > 0) {
      ...
      // Decibelberekening
      float pRef = 2e-5; // Referentiedruk in Pascal
      float decibels = 20.0 * log10(rms / pRef);
      // Resultaat weergeven
